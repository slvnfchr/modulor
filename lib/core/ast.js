...
ASTAnalyzer.create = function create() {
	const instance = new ASTAnalyzer();
	return instance;
};
...
function getFile(analyzer, file, module, encoding) {
	...
	return new Promise((resolve) => {
		...
		submodule.bundler = file.bundler;
		analyzer._transform(submodule, encoding, () => { // eslint-disable-line no-underscore-dangle
			resolve();
		});
	});
}
	...
	if (!(file instanceof File) || file.type !== File.types.JAVASCRIPT) {
		...
		if (!already) {
			fs.readFile(file.fullPath, { encoding }, (err, data) => {
				...
				if (file.bundler === constants.bundlers.REQUIREJS) {
