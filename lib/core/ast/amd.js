...
function isLoaderPlugin(ast) {
	let isLoader = false;
	let moduleName;
	let requireName;
	traverse(ast, (node) => {
		if (node.type === 'Property' && node.key.name === 'load' && node.value.type === 'FunctionExpression') {
			moduleName = node.value.params[0].name;
			requireName = node.value.params[1].name;
		} else if (requireName && node.type === 'CallExpression' && node.callee.name === requireName &&
			node.arguments.length > 0 && node.arguments[0].elements.length > 0 &&
			node.arguments[0].elements[0].type === 'Identifier' && node.arguments[0].elements[0].name === moduleName) {
			isLoader = true;  // module is loaded as is
			return false;
		}
		return true;
	});
