...
Bundler.prototype._flush = function flush(done) { // eslint-disable-line no-underscore-dangle
	if (this.registry.length > 0) {
		...
		// create bundle per loader plugin
		this.registry.filter((value) => value.isPlugin).forEach((value) => {
			bundles.push({ name: value.name, include: flattenDependencies(value, false) });
		});
		...
		if (this.type === constants.types.REQUIREJS) { // RequireJS specific
			...
			bundles.forEach((value, index) => {
				let bundle = value;
				if (this.configuration && this.configuration.baseUrl) Object.assign(bundle, { name: path.relative(this.configuration.baseUrl, bundle.name) });
				if (this.configuration && this.configuration.paths) bundle.name = getAlias(bundle.name, this.configuration.paths);
