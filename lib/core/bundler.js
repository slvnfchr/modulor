...
/**
 * RequireJS configuration inference from AMD modules distribution
 */
...
Bundler.create = function create(options) {
	const instance = new Bundler(options);
	this.options = options;
	return instance;
};
...
Bundler.prototype._flush = function flush(done) { // eslint-disable-line no-underscore-dangle
	const configuration = { paths: {}, bundles: [] };
	...
	// Exclude bundles from each other
	configuration.bundles.filter((value) => value !== common).forEach((bundle) => {
		if (!bundle.create) Object.assign(bundle, { exclude: configuration.bundles.filter((value) => value.name !== bundle.name).map((value) => value.name) });
	});
